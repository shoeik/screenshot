// 
// URLリスト（sitelist.txt）＋ --url=（コマンドライン引数）対応版
// 
...
// ------------------------------------------
// ★ コマンドライン引数の URL を取得
//    例）node screenshot.js --url=https://site.jp
// ------------------------------------------
...
async function processSite(TARGET_URL, browser) {
  ...
  for (const link of uniqueLinks) {
    ...
    if (CRAWL_DEPTH > 0 && depth > CRAWL_DEPTH) {
      console.log(`  ⏭ Skip (depth ${depth} > ${CRAWL_DEPTH}): ${link}`);
      continue;
    }
