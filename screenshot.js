...
// ので、Finderで管理する方式は一旦停止


// ver8からの変更点

// screenshot.js に、タグ生成＋JSON 保存

// capturePage() にタグ生成ロジックを追加
...
function loadScreenshotJSON() {
	...
	try {
		...
		console.error("❌ JSON 読み込みエラー:", err.message);
		return [];
	}
}


function saveScreenshotJSON(data) {
  fs.writeFileSync(JSON_PATH, JSON.stringify(data, null, 2), "utf8");
}

// 1回読み込んで、メモリ上で保持（高速）
let screenshotDB = loadScreenshotJSON();


// ============================================================
//  Utility: 1ページを撮影
// ============================================================
async function capturePage(pageLink, browser, outputDir, vp, slug, domain, folder) {
  ...
  try {
    ...
    const fileName = `${safeDomain}_${slug}_${vp.label}.webp`;
	...
	// 4) ドメイン判定
	if (domain.endsWith(".jp") || domain.endsWith(".co.jp")) {
		tags.push("jp_site");
	} else {
		tags.push("global_site");
	}
	...
	// 絶対パス → 相対パスへ変換
	const relativePath = path.join("..", filePath.replace(__dirname + "/", ""));
	...
	slug: slug,
	viewport: vp.label,
	path: relativePath,          // ← あなたの相対パス
	tags: tags,
	date: new Date().toISOString()
	};

	// screenshotDB が配列かどうかを保証
	if (!Array.isArray(screenshotDB)) {
	screenshotDB = [];
	}

	// 既存レコードの重複チェック（同じ fileName は消して更新）
	const idx = screenshotDB.findIndex(r => r.fileName === fileName);
	if (idx >= 0) {
	screenshotDB[idx] = record;  // 上書き
	} else {
	screenshotDB.push(record);   // 新規
	}

	saveScreenshotJSON(screenshotDB);
...
async function processSite(TARGET_URL, browser) {
  ...
  for (const link of uniqueLinks) {
    ...
    for (const vp of viewports) {
      await capturePage(link, browser, outputDir, vp, slug, domain, folder);
    }
  }
}
